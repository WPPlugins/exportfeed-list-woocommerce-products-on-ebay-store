...
class EBCPF_BasicFeed
{
    ...
    public function addAttributeDefault($attributeName, $value, $defaultClass = 'EBCPF_AttributeDefault')
    {
    ...
    {
        $className = 'EBCPF_FeedRule' . ucwords(strtolower($ruleClass));
    ...
    protected function continueFeed($category, $file_name, $file_path, $remote_category)
    {
        ...
        if ($this->has_footer && $done) {
            ...
            EBCPF_FeedActivityLog::updateFeedList($category, $remote_category, $file_name, $file_path, $this->providerName, $this->productCount);
        }
    ...
    function getFeedData($category, $remote_category, $file_name, $saved_feed = null)
    {
        ...
        $file_url = EBCPF_FeedFolder::uploadFolder() . $this->providerName . '/' . $file_name . '.' . $this->fileformat;
        $file_path = EBCPF_FeedFolder::uploadURL() . $this->providerName . '/' . $file_name . '.' . $this->fileformat;
        ...
        $this->shipping = new EBCPF_ShippingData($this);
        $this->taxData = new EBCPF_TaxationData($this);

        $this->logActivity('Initializing categories...');

        //Figure out what categories the user wants to export
        $this->categories = new EBCPF_ProductCategories($category);
        //Get the ProductList ready
        if ($this->productList == null)
            $this->productList = new EBCPF_ProductList();
        ...
        foreach ($this->relatedData as $relatedData)
            if (!isset($this->productList->relatedData[$relatedData[1]]))
                $this->productList->relatedData[$relatedData[1]] = new EBCPF_ProductSupplementalData($relatedData[0]);
...
//        $pfcore->feedType = 1;
        ...
        if ($this->auto_update_feedlist)
            EBCPF_FeedActivityLog::updateCustomFeedList( $cpf_custom_local_category, $cpf_custom_remote_category, $file_name, $file_path, $this->providerName, $this->productCount);
    ...
    function initializeOverrides($saved_feed)
    {
        ...
        $this->feedOverrides = new EBCPF_FeedOverride($this->providerName, $this, $saved_feed);
    ...
    function __construct($saved_feed = null)
    {
        ...
        $this->feed_category = new EBCPF_md5y();
...
class EBCPF_XMLFeed extends EBCPF_BasicFeed
{
...
class EBCPF_XML2Feed extends EBCPF_BasicFeed
{
...
class EBCPF_CSVFeed extends EBCPF_BasicFeed
{
...
class EBCPF_CSVFeedEx extends EBCPF_BasicFeed
{
...
class EBCPF_AggregateFeed extends EBCPF_BasicFeed
{
    ...
    function initializeAggregateFeed($id, $file_name)
    {
        ...
        $this->filename = EBCPF_FeedFolder::uploadFolder() . $this->providerName . '/' . $file_name . '.' . $this->fileformat;
        $this->file_url = EBCPF_FeedFolder::uploadURL() . $this->providerName . '/' . $file_name . '.' . $this->fileformat;
        $this->productCount = 0;
