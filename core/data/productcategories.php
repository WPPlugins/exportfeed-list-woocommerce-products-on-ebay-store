...
class EBCPF_ProductCategories {
	...
	public function getListW() {
		...
		$sql = $wpdb->prepare("SELECT taxo.term_id as id, term.name as title, taxo.count as tally 
			FROM $wpdb->term_taxonomy taxo
			LEFT JOIN $wpdb->terms term ON taxo.term_id = term.term_id
			WHERE taxo.taxonomy = %s",['product_cat']);
		...
		$sql = $wpdb->prepare("SELECT taxo.term_id as child_category, taxo.parent as parent_category 
			FROM $wpdb->term_taxonomy taxo
			WHERE taxo.taxonomy = %s",['product_cat']);
	...
	public function getListWe() {
		...
		$sql = $wpdb->prepare("SELECT taxo.term_id as id, term.name as title, taxo.count as tally 
			FROM $wpdb->term_taxonomy taxo
			LEFT JOIN $wpdb->terms term ON taxo.term_id = term.term_id
			WHERE taxo.taxonomy = %s",['wpsc_product_category']);
		$source_categories = $wpdb->get_results($sql);
