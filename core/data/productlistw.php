...
class EBCPF_ProductW extends EBCPF_PAProduct {

}

class EBCPF_MasterAttributeList {
...
class EBCPF_ProductList {
    ...
    public function loadProducts( $parent ) {
        ...
        if ( ! isset( $this->relatedData['brand'] ) && $parent->allowRelatedData ) {
            $this->relatedData['brand'] = new EBCPF_ProductSupplementalData( 'product_brand' );
        }
        if ( ! isset( $this->relatedData['tag'] ) && $parent->allowRelatedData ) {
            $this->relatedData['tag'] = new EBCPF_ProductSupplementalData( 'product_tag' );
        }
    ...
    protected function loadMasterAttributeList() {
        $this->masterAttributeList = new EBCPF_MasterAttributeList();
        global $wpdb;
    ...
    public function getProductListDefault( $parent, $remote_category ) {
        ...
        foreach ( $this->products as $index => $prod ) {
            ...
            //Prepare the item
            $item = new EBCPF_ProductW(); //extends PAProduct (basicfeed.php)
            ...
            $item->taxonomies = new EBCPF_TaxonomyList();
            foreach ( $item->attribute_details as $this_attribute ) {
                $this_attribute = explode( '=', $this_attribute );
                if ( count( $this_attribute ) > 1 ) {
