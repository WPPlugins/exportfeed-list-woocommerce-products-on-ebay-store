...
class EBCPF_SavedFeed {
	...
	private function feedLoaderW($id) {
		...
		$sql = $wpdb->prepare("SELECT f.*,description as local_category FROM $feed_table as f LEFT JOIN $wpdb->term_taxonomy t on ( f.category=term_id and t.taxonomy='product_cat'  ) WHERE f.id=%d",[$id]);
		...
		$this->local_category = '';
		$my_categories = explode(",", $this->category_id);
		$sql = $wpdb->prepare("SELECT tdesc.term_id, description, tname.name
			FROM $wpdb->term_taxonomy tdesc
			LEFT JOIN $wpdb->terms tname ON (tdesc.term_id = tname.term_id)
			WHERE tdesc.taxonomy=%s",['product_cat']);
