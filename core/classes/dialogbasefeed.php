...
class EBCPF_BaseFeedDialog
{

    public $blockCategoryList = false;
    public $options; //Array to be filled by constructor of descendant
    public $service_name = 'eBaySeller'; //Example only
    public $service_name_long = 'eBay List CSV Export'; //Example only
    public $feed_config;
    ...
    function attributeMappings()
    {
        ...
        global $pfcore;
        $FoundAttributes = new EBCPF_FoundAttribute();
...
<div class="clear"></div>
    ...
    function loadFeedConfig(){
        global $wpdb;
        $this->feed_config = get_option($this->service_name. '-ebcpf-settings-custom');
    }
    ...
    function categoryList($initial_remote_category)
    {
        if ($this->blockCategoryList)
            ...
            return '<span class="label" > ' . $this->service_name . ' Category : </span >
				  <span><input type = "text" name = "categoryDisplayText" class="text_big" id = "categoryDisplayText"  onclick = "showeBayCategories(\'eBaySeller\')" value = "' . $initial_remote_category . '" autocomplete = "off" placeholder = "Start typing for a category name" /></span >
    ...
    {
        global $pfcore;
        $this->advancedSettings = $pfcore->settingGet($this->service_name.'-ebcpf-settings');
        if ($source_feed == null) {
        ...
        $attrVal = array();
        $this->folders = new EBCPF_FeedFolder();
        $this->product_categories = new EBCPF_ProductCategories(); //used?
