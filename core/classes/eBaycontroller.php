...
class EBCPF_eBayController
{
	...
	var $eBayItemListing;
		...
		public function config(){
			$incPath = EBCPF_PATH .'/includes/EbatNs';
		...
		static function loadEbayClasses()
		{
			...
			//require_once (EBCPF_PATH  .'/includes/EbatNs/EbatNs_ServiceProxy.php');
			require_once (EBCPF_PATH .'/includes/EbatNs/EbatNs_Logger.php');
			require_once(EBCPF_PATH .'/includes/EbatNs/EbatNs_ServiceProxy.php');
			$incPath = EBCPF_PATH . '/includes/EbatNs/EbatNs_Session.php';
			require_once($incPath);
			require_once(EBCPF_PATH .'/includes/EbatNs/EbatNs_Client.php');
			require_once(EBCPF_PATH .'/includes/EbatNs/GetSessionIDRequestType.php');
			require_once(EBCPF_PATH .'/includes/EbatNs/GetTokenStatusRequestType.php');
			require_once(EBCPF_PATH .'/includes/EbatNs/FetchTokenRequestType.php');
			require_once(EBCPF_PATH .'/includes/EbatNs/GetUserRequestType.php');
			require_once(EBCPF_PATH .'/includes/EbatNs/GetUserPreferencesRequestType.php');
			require_once(EBCPF_PATH .'/includes/EbatNs/ItemType.php');
			require_once(EBCPF_PATH .'/includes/EbatNs/AddFixedPriceItemRequestType.php');
			require_once(EBCPF_PATH .'/includes/EbatNs/AddItemRequestType.php');
		...
		public function doFetchToken($account_id = false)
		{
			...
			if($token){
				if($account_id){
					$account = new EBCPF_eBayAccount( $account_id );
					$accoun->token = $token;
					$account->update();
				}
			}
		...
		public function initEbay($site_id , $sandbox_enabled , $token = false , $account_id = false )
		{
			...
			//init session
			//require_once(EBCPF_PATH .'/includes/EbatNs/EbatNs_Session.php');
...
//			}
			...
			if ( isset($_REQUEST['page']) && $_REQUEST['page'] == 'wplister-tools' ) {
				$sp->attachLogger( new EBCPF_EbatNs_Logger( false, 'db', $account_id, $site_id ) );
			}
		...
		**/
		...
		{
			/*require_once(EBCPF_PATH .'/core/classes/EBCPF_EbatNs_Logger.php');
			$this->sp->attachLogger(new EBCPF_EbatNs_Logger(false, 'db', $this->session->cpf_account_id , $this->siteId) );*/
		}// initLogger()
		...
		public function newAccount($ebay_token) {
			...
			require_once(EBCPF_PATH .'/core/model/eBayAccount.php');
			require_once(EBCPF_PATH .'/core/model/eBaySite.php');
			// create new account
			$account = new EBCPF_eBayAccount();
			// $account->title     = stripslashes( $_POST['wplister_account_title'] );
			$account->title        = 'My Account';
			$account->site_id      = $_REQUEST['site_id'];
			$account->site_code    = EBCPF_EbayController::getEbaySiteCode($_REQUEST['site_id'] );
			...
			$site = EBCPF_eBaySite::getSiteObj($account->site_id);
			$site->enabled = 1;
			$site->update();	
		...
		public function initEC( $account_id = null, $site_id = null ) { 
            ...
            // $account = new WPLE_eBayAccount( $account_id ); // not suitable to check if an account exists
				$account = EBCPF_eBayAccount::getAccount( $account_id );
				if ( $account ) {
